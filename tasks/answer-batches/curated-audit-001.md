...
- `cq_bean_319a398d`: independent review decision `revise`; score `68/100`; hard failures `unsupported_factual_claim`, `missing_version_boundary`. It was atomically demoted; the rewrite must correct Aware callback ordering and bound circular-reference/proxy behavior by Spring version and configuration.
- `cq_binlog_86a375fd`: independent review decision `revise`; score `77/100`; hard failures `unsupported_factual_claim`, `missing_version_boundary`, `missing_evidence`. It was atomically demoted; its rewrite must map two-phase commit, GTID/position, semi-sync, row-image and replica-apply facts to MySQL version/configuration sources.
- `cq_cms_collector_c069b541`, `cq_equals_hashcode_e7fe32f7`, `cq_g1_collector_828f806c`, `cq_hashmap_4d9f15d2`, `cq_hashmap_d74d2fd7`, `cq_io_multiplexing_6e30840f`: no evidence sidecar existed. The deterministic `missing_evidence` gate atomically demoted all six while preserving their bodies for future candidates; no AI score was fabricated.
