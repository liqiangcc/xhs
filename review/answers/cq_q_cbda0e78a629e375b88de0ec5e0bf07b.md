...
复习「项目中用什么 JDK 版本？介绍一下 JDK 8 和 JDK 17 的核心新特性。」时，应先给直接结论，再按定义与目标、核心链路、关键边界、验证方式展开，避免只罗列名词。

## 1 分钟版

复习「项目中用什么 JDK 版本？介绍一下 JDK 8 和 JDK 17 的核心新特性。」时，应先给直接结论，再按定义与目标、核心链路、关键边界、验证方式展开，避免只罗列名词。 关键检查点包括：先明确 JDK 版本与语言/运行时语义。；再讲对象、内存、并发或容器内部机制。；最后补充反例与可验证代码。。

## 3 分钟版

先界定题目中的概念、版本和约束，再按状态或数据流走一遍主链路；随后解释为什么采用该机制，并用失败路径、性能边界和替代方案校验结论。针对本题，应覆盖 java、jdk17、jdk8，最后给出可观测指标或最小实验。
...
- 问：项目中用什么 JDK 版本？介绍一下 JDK 8 和 JDK 17 的核心新特性的核心判断是什么？答：复习「项目中用什么 JDK 版本？介绍一下 JDK 8 和 JDK 17 的核心新特性。」时，应先给直接结论，再按定义与目标、核心链路、关键边界、验证方式展开，避免只罗列名词。
...
- 领域：Java基础 / 语言特性
- 关键实体：java、jdk17、jdk8
- 来源问法：项目中用什么 JDK 版本？介绍一下 JDK 8 和 JDK 17 的核心新特性。
