...
围绕「分布式 ID 生成架构：分析知乎场景下雪花算法、美团 Leaf、数据库号段模式的选型依据与取舍？」先报假设：用户量、峰值 QPS、数据量、延迟和一致性目标；再画入口、服务、存储与异步链路，指出关键分区键或幂等键；最后说明限流、缓存、重试、降级、监控和容量验证。
...
- 问：分布式 ID 生成架构：分析知乎场景下雪花算法、美团 Leaf、数据库号段模式的选型依据与取舍的核心判断是什么？答：这道场景题不能直接堆组件。先量化需求与容量，再给核心数据模型和读写链路，随后补一致性、幂等、过载保护、故障降级、可观测性和演进取舍。
...
- 来源问法：分布式 ID 生成架构：分析知乎场景下雪花算法、美团 Leaf、数据库号段模式的选型依据与取舍？
