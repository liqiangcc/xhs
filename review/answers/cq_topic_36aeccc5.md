<!-- xhs-answer: {"schema_version":"answer.v1","canonical_id":"cq_topic_36aeccc5","version":3,"status":"needs_update","updated_at":"2026-07-11","quality_tier":"curated_audit_failed","audit_failure":"missing_evidence"} -->
# 线程池的拒绝策略有哪些？
...
项目映射提示：按真实任务的重要性、阻塞比例和隔离要求拆池，记录 core/max、队列、超时、拒绝数和任务耗时。可补偿任务需有可靠落库/MQ 与幂等重试；关键同步任务应向调用方返回明确失败，不能把拒绝静默吞掉。

## 常见追问

- 问：任务提交流程是什么？答：先尝试创建 core 线程，再入队；队列满时尝试扩到 maximum，仍无法接收才执行拒绝策略。
- 问：为什么慎用无界队列？答：maximumPoolSize 可能失去扩容意义，流量持续高于处理能力时队列无限增长并拖垮内存与时延。
- 问：CallerRunsPolicy 有什么风险？答：用提交线程执行可形成反压，但可能阻塞请求线程、事件循环或锁持有者，放大级联延迟。
- 问：自定义拒绝策略怎么设计？答：按任务价值选择快速失败、降级或可靠转存，并记录池状态、trace 和告警；策略本身不能长时间阻塞。
