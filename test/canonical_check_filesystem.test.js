...
const { runCheck } = require('../scripts/commands/canonical');
...
test('production canonical check writes the report only when requested', () => {
    ...
    try {
        ...
        const noReport = runCheck({ root, noReport: true });
        assert.equal(typeof noReport?.then, 'undefined');
        assert.equal(noReport.schema_version, 'canonical_quality_report.v1');
        assert.equal(fs.existsSync(paths.qualityReport), false);
