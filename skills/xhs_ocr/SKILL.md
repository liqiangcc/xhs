...
5. **逐个处理**：每处理完 1 个笔记就提交，确保每次提交质量可控。
...
1. 当前笔记处理完成后，立即执行（`SafeToAutoRun = true`）：
   ```bash
   node scripts/commit_changes.js
   ```

2. **继续下一个**：回到步骤 A，重新执行 `find_ocr_needed.js --ids-only` 获取下一个任务。

3. **重复 A → B → C 循环**，每次只处理 1 个笔记，直到步骤 A 无输出或已处理完用户指定数量。
...
AI 执行：
```
1. node scripts/find_ocr_needed.js --ids-only  → 获取 ID 列表，取第 1 个
2. 处理该 ID：
