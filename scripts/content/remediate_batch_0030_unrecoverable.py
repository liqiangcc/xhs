#!/usr/bin/env python3
"""Retire batch-0030 singleton coding Questions whose exact contract is unrecoverable.

Source-first and idempotent: never invent a missing LeetCode problem statement from a
vague "Hot 100 hard variant" label. The caller rebuilds generated Question/index
projections after the SSOT mutation.
"""
...
ROOT = Path(".")
TARGETS = {
    "cq_q_6a1366ae78866d8d9f9b8ccdb1cc01b4": (
        "当前仓库的 tagged/structured source 只保留‘算法（二面）：力扣 Hot 100 困难题变种。’这一泛化描述；"
        "没有保存具体题名、题目正文、输入输出、数据范围、样例、变种条件或可执行 fixture。"
        "raw note payload 也没有提供能够恢复这些缺失约束的文本证据。Hot 100 中存在大量困难题及多种可能变种，"
        "任意补成某一道熟悉题都会把推测伪装成原题。获得更强原始来源前，该 singleton 无法恢复 strict-valid Coding 契约。"
    ),
}
...
def main() -> int:
    ...
    for canonical_id, explanation in TARGETS.items():
        ...
        if row.get("original_question") != "算法（二面）：力扣 Hot 100 困难题变种。":
    ...
    if not changed:
        print("Batch 0030 unrecoverable-question remediation already applied.")
        return 0
    ...
    print("Retired source-unrecoverable batch 0030 singleton:", ", ".join(sorted(TARGETS)))
    return 0
