#!/usr/bin/env python3
"""Reconcile and freeze Batch 0058 repository source contexts before answer writing."""
...
DATE = '2026-08-29'
SCHEDULED_TYPES = {
    'cq_q_6385688cc90c9b0c14dd4beaa2c72486': 'coding',
    'cq_q_68a77b01c3a999732bc21dc888503621': 'coding',
    'cq_q_51ccf7e50ca31c7d3361ba62e3d19e76': 'mechanism',
    'cq_q_1507c5b24de2c46f73908f3c80f3125c': 'concept',
    'cq_q_2720fe6386e36e70b5722d14ccc26773': 'concept',
    'cq_q_06024c66b90402a7ece9ccc7b7e6578d': 'concept',
    'cq_q_0e26bffea609c7de57b1404293587fba': 'project',
    'cq_q_177b93f3e94f4bfd3df72e3e1664310a': 'behavior',
    'cq_q_1e935072241ecdad7fccb3519290db2b': 'coding',
    'cq_q_3590292944e8b631aa2e0cf561c565e5': 'coding',
}
# These task entries are explicitly marked mixed_source_question_type; the current
# answer-type audit is authoritative if historical scheduling type has drifted.
TYPE_OVERRIDE_ALLOWED = {
    'cq_q_1507c5b24de2c46f73908f3c80f3125c',
    'cq_q_06024c66b90402a7ece9ccc7b7e6578d',
    'cq_q_0e26bffea609c7de57b1404293587fba',
}
PERSONAL_FACT_REQUIRED = {
    'cq_q_0e26bffea609c7de57b1404293587fba',
    'cq_q_177b93f3e94f4bfd3df72e3e1664310a',
}
...
def main() -> int:
    ...
    for scheduled_cid, scheduled_type in SCHEDULED_TYPES.items():
        ...
        if actual_type == scheduled_type:
            ...
            type_resolution = 'mixed_source_type_resolved_by_current_audit'
        else:
            raise SystemExit(f'{scheduled_cid}: answer type drift without mixed-source override: {scheduled_type} -> {actual_type}')
    ...
    type_overrides = [r for r in resolutions if r.get('type_resolution') == 'mixed_source_type_resolved_by_current_audit']
    ...
    inventory = {
        ...
        'mixed_source_type_resolution_count': len(type_overrides),
        ...
        'writer_rule': (
            ...
            'follow current Canonical ownership; mixed-source entries follow the current answer-type audit; and project/behavior '
            'entries marked personal_fact_verification_required must not fabricate user/project history when repository evidence is absent.'
        ),
    ...
    marker = (
        f'- [x] Batch 0058 scheduling reconciled against the current Question/Canonical/type SSOT and frozen in '
        f'`review/content_build/answer_batch_0058/source_inventory.json`: {len(active_targets)} active Canonicals, '
        f'{len(remapped)} stale assignments remapped to current ownership, {len(retired)} invalid/noise assignments kept retired, '
        f'{len(type_overrides)} mixed-source task types resolved by the current answer-type audit, and '
        f'{sum(1 for x in active_targets.values() if x["personal_fact_verification_required"])} personal-fact-sensitive targets explicitly guarded against fabrication. '
    ...
    print(f'PASS batch={BATCH} scheduled={len(SCHEDULED_TYPES)} active={len(active_targets)} remapped={len(remapped)} retired={len(retired)} mixed_type_resolutions={len(type_overrides)} personal_fact_targets={sum(1 for x in active_targets.values() if x["personal_fact_verification_required"])} source_questions={len(seen_source_qids)} current_types={dict(sorted(current_type_counts.items()))}')
    ...
    for item in active_targets.values():
        ...
        print(f"ACTIVE\t{item['canonical_id']}\t{item['answer_type']}\tpersonal={item['personal_fact_verification_required']}\t{source_text}")
    return 0
