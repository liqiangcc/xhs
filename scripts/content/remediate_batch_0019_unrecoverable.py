...
TARGETS = {
    ...
    "cq_q_3022a886d714c0fc8bc2fa4d0b7812a5": {
        "expected_original": "算法：深拷贝",
        "explanation": (
            "仓库原始笔记在“没咋问，一直在写题”上下文中只保留“深拷贝”三个字，"
            "没有保存输入类型、输出契约、需要支持的对象种类、循环引用、原型/属性描述符、Map/Set/Date 等边界，也没有指定语言 API。"
            "当前结构化 Question 把它扩成“算法：深拷贝”编码题，但现有来源不足以唯一恢复实现契约；"
            "在获得更强题面前应保留来源事实并排除，而不是自行选择一种 JavaScript/Java 深拷贝语义。"
        ),
    },
    "cq_q_3022eb4f8a66f69b09a2d019164f0bb5": {
        "expected_original": "算法手撕：最长回文子序列（Longest Palindromic Subsequence）。请对比动态规划（O(n^2)）方案与 Manacher（马拉车）算法在处理回文串时的思路差异",
        "explanation": (
            "仓库原始笔记只写“最长回文序列”，并记录第一种 O(n^2) 写法后第二种思路讲了马拉车算法。"
            "“最长回文序列”无法仅凭该文字唯一判定是子序列还是子串，而 Manacher 适用于回文子串，与当前结构化标题明确写成 Longest Palindromic Subsequence 存在语义冲突。"
            "来源没有保留足够信息消解这个冲突，因此不能静默选择子序列或子串契约；在获得更强题面前必须排除。"
        ),
    },
}
