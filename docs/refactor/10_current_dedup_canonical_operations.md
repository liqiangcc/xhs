...
- 若 `same/alias` 跨越一个已有 source Canonical，只有在**至少一个 reviewed Question 已属于指定 target Canonical**、且该 source Canonical 的**全部 Question 都在当前已审核 RelationCandidate 内**时，Application 才可把 source Canonical 合并进 target Canonical；
- 若 reviewed Questions 同时包含未归属 Question 和已有 source Canonical 的 Question，当前 Apply 不把两个 mutation 拼成隐式复合操作，必须 fail-closed；需要先把 source-first 范围与正式 ownership 收口后重新审核/应用；
- ownership resolution 得到的 reviewed Question ownership revision 会作为 opaque CAS 条件继续带入 Canonical Merge 的 preflight/commit；若解析之后出现第三 owner 或其它 ownership 漂移，写入前必须因 revision mismatch 失败；
...
- 若一个 Question 多 owner、跨多个非 target Canonical、target 不存在或没有 reviewed Question 作为语义桥、source Canonical 含任何未审核 Question，Apply 必须 fail-closed；需要扩大/重做 source-first review，不能顺手移动未审核内容；
...
Application reloads current ownership
  ...
  ├─ one source Canonical + reviewed target bridge + fully-reviewed source
  │    → reviewed Canonical consolidation
  │       └─ ownership revisions participate in merge CAS
  └─ mixed unowned/source, unrelated target, or unreviewed source member
       → FAIL CLOSED
```
...
当显式 `same/alias` Decision 证明两个已存在 Canonical 需要收敛时，`dedup apply` 会在 Application 内根据**当前 ownership**选择 reviewed consolidation，并调用与 `canonical merge` 相同的正式事务能力；caller 仍然只能指定 target，不能指定 source，也不能让未进入本次 review 的 source members 被连带迁移。跨 Canonical 收敛还要求当前 RelationCandidate 中至少一个 Question 已属于 target，防止把完整 reviewed source 错接到一个与本次审核无关的既有 Canonical。
...
随后仍必须完成 explicit Decision；不得因为历史 Canonical 状态、旧 review 记录或 similarity 结果直接应用关系。若 Decision 为 `same/alias` 且两题属于不同 Canonical，`dedup apply` 只能在 target 有本次已审核 Question 作为桥、且 source Canonical 的全部成员都已被本次 review 覆盖时收敛 Canonical，否则 fail-closed。若一侧仍未归属而另一侧已在 source Canonical，当前也 fail-closed，不把“创建/扩展 + merge”偷偷组合成一次授权。
...
6. 跨 Canonical consolidation 必须有本次 reviewed Question 已属于 target；source Canonical 包含未审核 Question、reviewed 集合混有未归属 Question，或 ownership 在 resolve 后漂移时，不扩大授权，必须 fail-closed 并先扩展/重做 review；
7. 只有不涉及新增关系判断的既有 Canonical 维护才直接走 Merge/Split；
8. 正式写入前保留 explicit review、freshness、ownership CAS 与 Canonical mutation CAS；
...
target 必须有本次 reviewed Question 作为跨 Canonical 语义桥
source Canonical 含未审核 Question时 reviewed consolidation fail-closed
mixed unowned/source reviewed ownership 不执行隐式复合 mutation
reviewed ownership revision 参与 merge preflight/commit CAS
