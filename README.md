...
The `canonical accept` CLI and its executable runtime/mutation/test-support paths have been retired. Historical `canonical_candidates.v1` remains only as final path/data cleanup residue; new Suggest flows, GitHub Actions, Agents, and normal manual work must use Suggest -> Decide -> Apply. See `docs/refactor/09_legacy_canonical_accept_boundary.md`.
...
The `canonical accept` CLI, runtime path, MutationPlan operation, and shared in-memory candidate test support are retired. Only inert path/data cleanup residue remains. See `docs/refactor/09_legacy_canonical_accept_boundary.md`.
