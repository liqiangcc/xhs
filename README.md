...
The `canonical accept` CLI has been retired. Historical/manual `canonical_candidates.v1` compatibility remains internal temporarily while the staged runtime removal proceeds; new Suggest flows, GitHub Actions, Agents, and normal manual work must use Suggest -> Decide -> Apply. See `docs/refactor/09_legacy_canonical_accept_boundary.md`.
...
The `canonical accept` CLI is retired. Its remaining internal compatibility chain is being removed in staged slices and is not a supported user entrypoint. See `docs/refactor/09_legacy_canonical_accept_boundary.md`.
