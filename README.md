...
`canonical accept --candidate-id ...` still exists only for historical or manually supplied `data/manifests/canonical/canonical_candidates.json`. New Suggest flows and GitHub Actions must not generate `canonical_candidates.v1`; see `docs/refactor/09_legacy_canonical_accept_boundary.md`.
...
`canonical accept` is also a compatibility entrypoint, not the default path for newly detected relations. See `docs/refactor/09_legacy_canonical_accept_boundary.md`.
